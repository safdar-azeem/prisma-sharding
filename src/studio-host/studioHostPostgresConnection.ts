 ...
 *
 * The executor itself comes from `@prisma/studio-core`; the host does not
...
interface StudioCorePostgresJsModule {
  ...
  /**
   * Present in newer `@prisma/studio-core` releases. When absent the
...
export const createStudioHostPostgresConnectionFactory = (
  ...
  const {
    ...
    loadStudioCore = () =>
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      require('@prisma/studio-core/data/postgresjs') as StudioCorePostgresJsModule,
  } = options;
